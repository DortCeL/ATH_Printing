			...
			<div className='container relative'>
							...
							<table className='w-full min-w-[520px] border-collapse text-left'>
								...
								<tbody>
									{m.machines.map((machine, i) => {
										const featured = "featured" in machine && Boolean(machine.featured);
										return (
											<tr
												key={i}
				...
				<div className='flex flex-wrap gap-3 mb-10 md:mb-14'>
					{m.extras.map((extra, i) => {
						...
						return (
							<Reveal key={i} variant='left' delay={100 + i * 120}>
								<div
				...
				<Reveal variant='scale' delay={60}>
					<div className='rounded-2xl border border-accent/30 bg-gradient-to-br from-accent/[0.12] via-background to-primary/[0.08] p-4 sm:p-5 md:p-6'>
						...
						<div className='grid grid-cols-3 sm:grid-cols-6 gap-2 sm:gap-3'>
							{m.capacity.map((c, i) => (
								<Reveal key={c.size} variant='up' delay={80 + i * 70}>
									<div className='rounded-xl bg-card border border-border shadow-sm px-2.5 py-3 sm:px-3 sm:py-3.5 text-center'>
