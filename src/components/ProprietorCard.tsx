...
const ProprietorCard = ({ image, alt, className = "" }) => {
	const { t } = useLanguage();
	const p = translations.overview.proprietor;
	...
	return (
		<div
			className={`flex flex-col sm:flex-row items-center sm:items-start gap-6 sm:gap-8 p-5 sm:p-6 bg-card border border-white/10 rounded-2xl shadow-lg shadow-black/20 ${className}`}
		>
			...
			<div className='flex-1 text-center sm:text-left py-4 sm:py-6 md:py-10'>
				...
				<p className='text-accent font-medium text-md mb-3'>{t(p.title)}</p>
				...
				<div className='flex justify-center sm:justify-start gap-4 mt-4'>
					<a
						href={proprietor.facebook}
						target='_blank'
						rel='noopener noreferrer'
						className='inline-flex items-center gap-2 text-muted-foreground hover:text-accent transition-colors'
					>
						<Facebook size={20} />
						<span>{t(p.facebook)}</span>
					</a>
				</div>
			</div>
		</div>
